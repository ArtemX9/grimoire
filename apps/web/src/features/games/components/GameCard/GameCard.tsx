  ...
  function renderCover() {
    return (
      <div className='relative aspect-[3/4] w-full overflow-hidden bg-grimoire-hover'>
        ...
        <span
          className={cn(
            'absolute right-2 top-2 rounded-full px-2 py-0.5 font-sans text-[10px] font-medium',
  ...
  function renderInfo() {
    return (
      <div className='flex flex-col gap-1 p-2.5'>
        <h3 className='font-grimoire text-sm leading-snug text-grimoire-ink line-clamp-2'>
          {game.title}
        </h3>
        <div className='flex items-center justify-between'>
          {game.genres[0] ? (
            <span className='font-sans text-[10px] text-grimoire-muted truncate max-w-[70%]'>
              {game.genres[0]}
            </span>
          ) : (
            <span />
