...
test.describe('Unmapped games — mapping a new game', () => {
  ...
  test(
    'mapping an unmapped Xbox game removes it from the list and adds it to the library with Xbox icon',
    async ({ page }) => {
      ...
      // -----------------------------------------------------------------------
      const firstResult = mapDialog.locator('button').filter({ hasText: /Elden Ring/i }).first();
      ...
      const libraryCard = page
        ...
        .filter({ hasText: IGDB_RESULT.name })
        .first();
...
test.describe('Unmapped games — mapping onto an existing library entry', () => {
  ...
  test(
    'mapping an unmapped Xbox game onto an existing PSN entry adds Xbox platform icon alongside PSN icon',
    async ({ page }) => {
