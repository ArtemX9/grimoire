...
test.describe('Game status transitions', () => {
  ...
  // -------------------------------------------------------------------------
  test('status changes persist on detail page and are reflected on the library card', async ({ page }) => {
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    const hkCard = page.locator('a[href^="/games/"]').filter({ hasText: SEEDED_GAME_TITLE }).first();
    ...
    await page.route('**/api/v1/games**', (route) => {
      ...
      const isListEndpoint = /\/api\/v1\/games$/.test(url.pathname) || /\/api\/v1\/games\?/.test(url.href);
    ...
    // -----------------------------------------------------------------------
    const updatedCard = page.locator('a[href^="/games/"]').filter({ hasText: SEEDED_GAME_TITLE }).first();
    await expect(updatedCard).toBeVisible({ timeout: 10_000 });

    // The status badge is rendered inside the card cover as a <span>.
    await expect(updatedCard.locator('span').filter({ hasText: 'COMPLETED' })).toBeVisible({ timeout: 10_000 });
  });
});
